import media
import fresh_tomatoes
import requests

api_key = "edacb7eeacf9c2be67d32057dc199a0f"

list_of_movies = requests.get('https://api.themoviedb.org/3/movie/popular?api_key='+api_key+'&language=en-US&page=1')

if list_of_movies.status_code != 200:
    # This means something went wrong.
    print(list_of_movies.status_code)

array_of_movies = []

# get poster url
def get_poster_url_by(poster_path):
    poster_url = "https://image.tmdb.org/t/p/w600_and_h900_bestv2"+poster_path
    return poster_url

# get trailer url
def get_trailer_url_by(movie_id):
    # list_of_trailers = requests.get("https://api.themoviedb.org/3/movie/346364/videos?api_key="+api_key+"&language=en-US")

    # print(list_of_trailers.json()["results"][0]["key"])
    #
    trailers_api_url = "https://api.themoviedb.org/3/movie/"+str(movie_id)+"/videos?api_key="+api_key+"&language=en-US"
    trailer_object = requests.get(trailers_api_url)
    print(trailer_object)
    trailer_url = "https://www.youtube.com/watch?v=" + str(trailer_object.json()["results"][0]["key"])
    return trailer_url

# create a movie object
def get_movie_data(movie):
    print(movie)
    data_movie = media.Movie(
        movie["title"],
        movie["overview"],
        get_poster_url_by(movie["poster_path"]),
        get_trailer_url_by(movie["id"]),
        movie["vote_average"],
        movie["popularity"]
        # str(movie["release_date"]).encode('utf-8', 'ignore').decode('utf-8')
    )
    return data_movie

# populates the array with movie objects
for movie_item in list_of_movies.json()["results"]:
    array_of_movies.append(get_movie_data(movie_item))

#
# #A Wrinkle in Tim - title, storyline, poster_image_url, trailer_youtube_url, director, writers, stars
# a_wrinkle_in_tim = media.Movie(
#     "A Wrinkle in Tim",
#     "After the disappearance of her scientist father, three peculiar beings send Meg, her brother, and her friend to space in order to find him.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMjMxNjQ5MTI3MV5BMl5BanBnXkFtZTgwMjQ2MTAyNDM@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=UhZ56rcWwRQ",
#     "Ava DuVernay",
#     "Jennifer Lee (screenplay by), Madeleine L'Engle (based upon the novel by)",
#     "Reese Witherspoon, Chris Pine, Gugu Mbatha-Raw ")
#
# # The Shape of Water
# the_shape_of_water = media.Movie(
#     "The Shape of Water",
#     "At a top secret research facility in the 1950s, a lonely janitor forms a unique relationship with an amphibious creature that is being held in captivity.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BNGNiNWQ5M2MtNGI0OC00MDA2LWI5NzEtMmZiYjVjMDEyOWYzXkEyXkFqcGdeQXVyMjM4NTM5NDY@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=XFYWazblaUA",
#     "Guillermo del Toro",
#     "Guillermo del Toro (screenplay by), Vanessa Taylor (screenplay by)",
#     "Sally Hawkins, Octavia Spencer, Michael Shannon")
#
# # Three Billboards Outside Ebbing
# three_bilboards_outsite_ebbing = media.Movie(
#     "Three Billboards Outside Ebbing",
#     "A mother personally challenges the local authorities to solve her daughter's murder when they fail to catch the culprit.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BZTZjYzU2NTktNTdmNi00OTM0LTg5MDgtNGFjOGMzNjY0MDk5XkEyXkFqcGdeQXVyMTMxODk2OTU@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=Jit3YhGx5pU",
#     "Martin McDonagh",
#     "Martin McDonagh",
#     "Frances McDormand, Woody Harrelson, Sam Rockwell")
#
# # Tiger Zinda Hai
# tiger_zinda_hai = media.Movie(
#     "Tiger Zinda Hai",
#     "Inspired by real events, Tiger Zinda Hai is a sequel to the blockbuster Ek Tha Tiger, and an espionage action thriller that follows an adventurous rescue mission in Iraq.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMTYzOTk4OTA5N15BMl5BanBnXkFtZTgwMzUzNDIwNDI@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=ePO5M5DE01I",
#     "Ali Abbas Zafar",
#     "Neelesh Misra (story), Ali Abbas Zafar (story)",
#     "Salman Khan, Katrina Kaif, Sajjad Delafrooz")
#
# # Call Me by Your Name
# call_me_by_your_name = media.Movie(
#     "Call Me by Your Name",
#     "In Northern Italy in 1983, seventeen year-old Elio begins a relationship with visiting Oliver, his father's research assistant, with whom he bonds over his emerging sexuality, their Jewish heritage, and the beguiling Italian landscape.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BNDk3NTEwNjc0MV5BMl5BanBnXkFtZTgwNzYxNTMwMzI@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=Z9AYPxH5NTM",
#     "Luca Guadagnino",
#     "James Ivory (screenplay by), Andre Aciman (based on the novel by)",
#     "Armie Hammer, Timothee Chalamet, Michael Stuhlbarg"
# )
#
# # Phantom Thread
# phantom_thread = media.Movie(
#     "Phantom Thread",
#     "Set in 1950's London, Reynolds Woodcock is a renowned dressmaker whose fastidious life is disrupted by a young, strong-willed woman, Alma, who becomes his muse and lover.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMWJkNzBkM2UtYWFlMC00NmEwLTgxOGUtMjVmMzYyZjgyMmEzXkEyXkFqcGdeQXVyMjM4NTM5NDY@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=xNsiQMeSvMk",
#     "Paul Thomas Anderson",
#     "Paul Thomas Anderson",
#     "Vicky Krieps, Daniel Day-Lewis, Lesley Manville"
# )
#
# # Darkest Hour
# darkest_hour = media.Movie(
#     "Darkest Hour",
#     "During the early days of World War II, the fate of Western Europe hangs on the newly-appointed British Prime Minister Winston Churchill, who must decide whether to negotiate with Hitler, or fight on against incredible odds.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMTgwNzE3NDcwNF5BMl5BanBnXkFtZTgwMjI2MzI0NDM@._V1_UY268_CR2,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=LtJ60u7SUSw",
#     "Joe Wright",
#     "Anthony McCarten",
#     "Gary Oldman, Lily James, Kristin Scott Thomas"
# )
#
# # The Post
# the_post = media.Movie(
#     "The Post",
#     "A cover-up that spanned four U.S. Presidents pushed the country's first female newspaper publisher and a hard-driving editor to join an unprecedented battle between the press and the government.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMjQyMjEwOTIwNV5BMl5BanBnXkFtZTgwOTkzNTMxNDM@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=nrXlY6gzTTM",
#     "Steven Spielberg",
#     "Liz Hannah, Josh Singer",
#     "Meryl Streep, Tom Hanks, Sarah Paulson"
# )
#
# # Avengers: Infinity War
# avengers_infinity_war = media.Movie(
#     "Avengers: Infinity War",
#     "The Avengers and their allies must be willing to sacrifice all in an attempt to defeat the powerful Thanos before his blitz of devastation and ruin puts an end to the universe.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMTc0MjA1OTMxOV5BMl5BanBnXkFtZTgwMzM1NDcyNDM@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=6ZfuNTqbHE8",
#     "Anthony Russo, Joe Russo",
#     "Christopher Markus (screenplay), Stephen McFeely (screenplay) ",
#     "Karen Gillan, Pom Klementieff, Tessa Thompson"
# )
#
# # Suburbicon
# suburbicon = media.Movie(
#     "Suburbicon",
#     "As a 1950s suburban community self-destructs, a home invasion has sinister consequences for one seemingly normal family.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMTA3MjA1NDkxMTReQTJeQWpwZ15BbWU4MDU2Njg3NDMy._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=IYga2m0V2O0",
#     "George Clooney",
#     "Joel Coen, Ethan Coen ",
#     "Matt Damon, Julianne Moore, Oscar Isaac"
# )
#
# # Den of Thieves
# den_of_thieves = media.Movie(
#     "Den of Thieves",
#     "A gritty crime saga which follows the lives of an elite unit of the LA County Sheriff's Dept. and the state's most successful bank robbery crew as the outlaws plan a seemingly impossible heist on the Federal Reserve Bank.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMzYyODcwMDAyM15BMl5BanBnXkFtZTgwOTA4MjIwNDI@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=FKd_ks0rdAM",
#     "Christian Gudegast",
#     "Christian Gudegast (screenplay), Christian Gudegast (story)",
#     "Gerard Butler, Jordan Bridges, Pablo Schreiber "
# )
#
#
#
# # I, Tonya
# i_tonya = media.Movie(
#     "I, Tonya",
#     "Competitive ice skater Tonya Harding rises amongst the ranks at the U.S. Figure Skating Championships, but her future in the activity is thrown into doubt when her ex-husband intervenes.",
#     "https://images-na.ssl-images-amazon.com/images/M/MV5BMjI5MDY1NjYzMl5BMl5BanBnXkFtZTgwNjIzNDAxNDM@._V1_UX182_CR0,0,182,268_AL_.jpg",
#     "https://www.youtube.com/watch?v=KuDQOMICfr0",
#     "Craig Gillespie",
#     "Steven Rogers",
#     "Margot Robbie, Sebastian Stan, Allison Janney"
# )


# array of movies
#
# movies = [
#     the_shape_of_water,
#     three_bilboards_outsite_ebbing,
#     tiger_zinda_hai,
#     phantom_thread,
#     i_tonya,
#     avengers_infinity_war,
#     darkest_hour,
#     the_post,
#     call_me_by_your_name,
#     suburbicon
#     ]

fresh_tomatoes.open_movies_page(array_of_movies)
